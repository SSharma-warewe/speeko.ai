...
- **Workflow / speech:** `tasks/workflow-task` wraps `AgentTask.create`: realtime **task `onEnter` `generateReply`s the opening** after handoff; inbound script may cache location → timing → BHK/budget via `session.say` + `StopResponse` on task `onUserTurnCompleted`; skip realtime, `finishWorkflowTask` goodbyes on the task then `complete()`.
