...
| P2 | `whatsapp` | **Done** | Generate stores hash only; rotate invalidates old token; at-least-one id; 409 on phone/WABA collision; Meta GET verify (challenge vs 403); POST persists any JSON body (empty → `{ raw: null }`) and routes by phone_number_id / WABA (numeric ids coerced); unknown ids 200 + null org; org list is latest 50 for that org plus unmatched null-org posts. Receptionist (`whatsapp-agent`): text-only reply on the `WHATSAPP_URL` line, duplicate message id skipped, missing OpenRouter/WhatsApp config does not throw, ingest stays 200 when the agent rejects, `/new` resets the in-memory session and sends a canned reply without Luna |
