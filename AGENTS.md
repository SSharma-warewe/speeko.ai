...
- `whatsapp_webhook_events` — one row per Meta POST: `payload` JSONB (any JSON body; empty body stored as `{ raw: null }`), `event_type` (first `entry[].changes[].field`, else `unknown`), `received_at`, optional `organization_id` (SET NULL; null when ids do not match a config). Phone number id and WABA id accept numeric Graph ids. Always HTTP 200 after persist. Inbound **text** on the phone number id inside `WHATSAPP_URL` is answered afterward by the WhatsApp receptionist (Google ADK + OpenRouter `openai/gpt-5.6-luna`); that reply does not delay the 200. Exact inbound `/new` (any letter case) drops that sender's in-memory session and sends a canned confirmation without calling Luna. Other lines stay store-only.
...
# Optional: official Python STTRealtime sidecar for sarvam/saaras-v3-realtime
# npm run start:stt-sarvam   # then SARVAM_STT_PLUGIN_URL=ws://127.0.0.1:8091/stt
...
| `OPENROUTER_API_KEY` | API | Optional. WhatsApp receptionist calls OpenRouter `openai/gpt-5.6-luna` through Google ADK. Never logged, never in Vite. Missing key skips the reply; the webhook still returns 200. |
...
| POST | `/api/webhooks/whatsapp` | public — persist any JSON body (empty → `{ raw: null }`); 200 immediately. After save, inbound text on the `WHATSAPP_URL` phone number id is answered by the receptionist (not awaited). Exact `/new` resets that sender's in-memory session |
...
| `whatsapp` | Org WhatsApp webhook config (hashed verify token + phone/WABA routing) + public Meta GET verify / POST ingest. Persist first and return 200. `whatsapp-agent` then replies to inbound text on the platform line |
...
- **Hangup:** successful task completion auto-ends the session and deletes the LiveKit room (SIP drops). **Pipeline:** hang up immediately; parent **onExit** `session.say` plays the canned/custom goodbye. **Realtime:** `finishWorkflowTask` `generateReply`s the canned goodbye (`buildRealtimeClosingInstructions`, `toolChoice: 'none'`, never `allowInterruptions: false`) **on the task** before `complete()`, waits (`waitForRealtimeUtterance` — `min` 2.5s / timeout **3.5s**, do **not** trust `waitForPlayout` or xAI `agentState`; Hindi canned line when the persona is Hindi), then parent `hangUpCall`; parent does not `generateReply` goodbye (that ran on a wiped parent context and re-greeted). onExit stays silent (`session.say` does not play on native realtime). Mid-call hangup uses the `end_call` tool. Shared helper: `apps/worker/src/speech/hangup.ts`.
...
- `whatsapp` is webhook ingest: org-user generate/rotate (`POST /users/whatsapp/webhook-config`) + list (`GET /users/whatsapp/webhook-events`, latest 50 for the JWT org plus unmatched null-org posts) + public Meta GET/POST `/webhooks/whatsapp` (no JWT). Persist raw JSONB, then return 200. GET verify echoes `hub.challenge` as `text/plain`. Callback URL uses `API_PUBLIC_URL` (then public `API_BASE_URL`, then `RAILWAY_PUBLIC_DOMAIN`) + `/api/webhooks/whatsapp`. Portal Integrations → WhatsApp tab generates/copies the callback URL and verify token and lists those posts. After a successful save, `whatsapp-agent` replies to inbound `type: text` messages whose `phone_number_id` matches the id in `WHATSAPP_URL`. The reply is a Cloud API text body (`messaging_product`, `to`, `type: text`) using `WHATSAPP_API_KEY`. The model is Google ADK `LlmAgent` `warewe_receptionist` on OpenRouter `openai/gpt-5.6-luna` (`OPENROUTER_API_KEY`, `adk-llm-bridge`). Luna reasoning (`thought` parts) is stripped before the session stores the turn and is never sent as the WhatsApp body. No booking tools yet — the instruction must not claim a meeting was booked. Sessions are in-memory, keyed by the sender digits (lost on restart; one memory per API process). Exact inbound `/new` (any letter case) deletes that ADK session and sends a canned confirmation without calling Luna; `/new` still works when `OPENROUTER_API_KEY` is missing. A missing WhatsApp key skips the send. Do not log `OPENROUTER_API_KEY`, the WhatsApp token, or message bodies.
...
10. Keep STT/LLM/TTS/realtime catalogs in `@call-agent/contracts`; worker `model-builder` constructs from those specs (`null` = Gemma / Inworld / Deepgram).
...
24. **Call cost analysis goes through `PriceService`** (published LiveKit list prices, `markup: 0`; Sarvam STT/TTS uses Sarvam’s INR list converted to USD in `price.catalog.ts`). Do not add Speeko margin. Org-user cost APIs must scope by JWT `orgId` (never a client `organizationId`). Recompute stays admin-only. Bump `PRICE_CATALOG_AS_OF` in `price.catalog.ts` when LiveKit rates change.
25. **Shared wire types and catalogs live in `packages/contracts` (`@call-agent/contracts`).** Add a new tool id, task key, call status, failure code, delivery mode, TTS/STT/LLM/realtime model/voice, speech language, job-metadata field, or get-demo lead-quality helper there first. API Nest DTO **classes** stay in `apps/api` (class-validator / Swagger); they import enums/consts from the package. Portal and worker import the same types — do not copy catalogs into `apps/portal/src/lib/api.ts` or worker `tool-ids.ts`. Build with `npm run build:contracts` (also run by `build:api` / `build:worker`). Vite apps alias the package to source.
...
| P2 | `price` | **Done** | Gemma/Nova-3/Inworld list-price math, web vs SIP room lines, self-hosted vs Cloud agent session, 10s min, unknown models, attempt rollup, admin summary SQL, recompute skip/404, org-user summary JWT-org scoped, Sarvam Saaras/Bulbul INR list converted to USD |
...
| P2 | `whatsapp` | **Done** | Generate stores hash only; rotate invalidates old token; at-least-one id; 409 on phone/WABA collision; Meta GET verify (challenge vs 403); POST persists any JSON body (empty → `{ raw: null }`) and routes by phone_number_id / WABA (numeric ids coerced); unknown ids 200 + null org; org list is latest 50 for that org plus unmatched null-org posts. Receptionist (`whatsapp-agent`): text-only reply on the `WHATSAPP_URL` line, Luna `thought` parts dropped from the sent body and from the stored turn, duplicate message id skipped, missing OpenRouter/WhatsApp config does not throw, ingest stays 200 when the agent rejects, `/new` resets the in-memory session and sends a canned reply without Luna |
...
| P3 | Portal / web | Partial | Call-tape **Outcome** badge mapper (`callDisplayOutcome`: lifecycle vs `taskResult.outcome`; leftover `NO_ANSWER` ignored). Remaining: Vitest/Playwright for SPA flows |
...
- Do not run the LiveKit worker via Nest (`nest start worker` — it is not a Nest project). Dev: `tsx apps/worker/src/main.ts`; prod: `node dist/apps/worker/main.js start` after `npm run build:worker`.
