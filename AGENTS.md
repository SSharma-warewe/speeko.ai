...
| `OPENROUTER_API_KEY` | worker | Optional. Required only when an agent uses Gemini 3.1 Flash TTS (`google/gemini-3.1-flash-tts-preview`). Never in LiveKit metadata or Vite. Missing key **fails** Gemini jobs (no silent Inworld fallback). |
...
`model` is the **LLM** override. `ttsModel` is the **speech** catalog id (`inworld/inworld-tts-2`, `fishaudio/s2.1-pro-free`, `google/gemini-3.1-flash-tts-preview`; `null` = Inworld). Switching `ttsModel` changes the allowed `voice` set (catalog in `@call-agent/contracts` `tts.ts`). Fish uses LiveKit Inference (`fishaudio/s2.1-pro-free`; OpenRouter slug `fish-audio/s2.1-pro-free:free` is accepted and stored as the LiveKit id). Gemini TTS is **not** on LiveKit Inference — the worker calls OpenRouter `/api/v1/audio/speech` with `OPENROUTER_API_KEY` and **streams PCM as the HTTP body arrives** (do not buffer `arrayBuffer()`; LiveKit aborts if the TTS stream idles ~10s after first audio). Cancelled/aborted OpenRouter reads must not retry. Gemini TTFB is still higher than Inworld/Fish Inference streaming. `temperature` is **LLM** reply randomness. `speakingRate` maps to Inworld `speaking_rate` or Fish `speed`; `deliveryMode` is Inworld-only. Gemini OpenRouter usage is billed on OpenRouter, not the LiveKit cost snapshot.
...
- **Builders:** `prompt-builder` (persona + onEnter instructions + onExit spoken line), `model-builder` (LLM `temperature` + TTS from `ttsModel` / `voice` / `speakingRate` / `deliveryMode`; Inworld + Fish via LiveKit Inference, Gemini via OpenRouter), `voice-builder` (session), `tool-builder` → `ToolRegistry`, `task-builder` → `TaskRegistry`, orchestrated by `agent-builder`.
...
- **Tasks:** LiveKit `voice.AgentTask` under `apps/worker/src/tasks/*`. Parent agent **onEnter** speaks opening (configurable), then runs `task.run()`; task owns workflow + completion tools (no opening `generateReply`). `composeTaskInstructions` copies `buildPersonaPrompt` into the task system prompt so company facts survive the handoff (`excludeInstructions: true` on copied history so it is not duplicated). Structured result in `userData.taskResult`. `userData.taskCompleted = true` only after `task.run()` **resolves** (not because `taskResult` is set).
...
| P3 | Worker (`apps/worker`) | Partial | SIP answer wait + unanswered shutdown (`sip-answer`, `shutdown-status` including `taskCompleted`); job entry `runAgentJob` (web skips SIP wait, **outbound** waits for SIP answer, **inbound ringing skips `waitForSipAnswer` then `session.start`**, unanswered `failedEarly` skips shutdown complete, 30-job sequential feed with timeout instead of real dial/session, **inbound SIP ensure then complete**); `buildClosingSpeech` (silent/custom/default); **`composeTaskInstructions` copies persona into every task prompt**; `interview_booking` identity-then-congratulate-then-book instructions (tool-agnostic); job metadata `speakingRate`/`deliveryMode`/`ttsModel`; TTS/LLM option helpers + OpenRouter Gemini TTS request shape; **complete callback timeout + retry** (`call-callback`) + **inbound ensure**. Remaining: other prompt/tool/task builders, hangup helpers — no LiveKit cloud in unit tests |
