...
| POST | `/api/webhooks/whatsapp` | public — persist any JSON body (empty → `{ raw: null }`); 200 immediately. After save, inbound text on the `WHATSAPP_URL` phone number id is answered by the receptionist (not awaited) |
